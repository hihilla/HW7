...
public class KMeans {
	 ...
	 * It should initialize centroids (by calling initializeCentroids) and 
	 * run the K-Means algorithm (which means to call findKMeansCentroids methods). 
	 * @param instances
	 */
	...
	/**
	 * Initialize the centroids by selecting k random instances from the training set 
	 * and setting the centroids to be those instances.
	 * @param instances
	 */
	 ...
	 *	centroids have not moved much from their previous location,
 	 *	the cost function did not change much, or you have reached a preset number of iterations. 
	...
	/**
	 * a.	Input: 2 Instance objects – one is an instance from the dataset and one is a centroid
	 *  (if you're using different data structure for the centroid, feel free to change the input).
	 * b.	Output: should calculate the squared distance between the input
	 *  instance and the input centroid
	...
	/**
	 * Input: Instance
	 * Output: the index of the closest centroid to the input instance
	 * @param instance
	 * @return
	 */
	...
	/**
	 * Output: should replace every instance in Instances by the centroid
